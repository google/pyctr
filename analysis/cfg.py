...
# TODO(mdanatg): The notion of 'statements' below is inaccurate.
...
class Node(object):
  ...
  def __repr__(self):
    if isinstance(self.ast_node, gast.FunctionDef):
      ...
      return parsing.ast_to_source(self.ast_node.context_expr).strip()
    return parsing.ast_to_source(self.ast_node).strip()
...
# TODO(mdanatg): Rename to DataFlowAnalyzer.
# TODO(mdanatg): Consider specializations that use gen/kill/transfer abstractions.
...
class GraphBuilder(object):
  ...
  # TODO(mdanatg): Perhaps detail this in a markdown doc.
  # TODO(mdanatg): Add exception support.
  ...
  def reset(self):
    ...
    # TODO(mdanatg): Too many primitives. Use classes.
    self.leaves = set()
  ...
  def _connect_jump_to_finally_sections(self, node):
    ...
    # TODO(mdanatg): Should garbage-collect finally_section_subgraphs.
    return cursor
  ...
  def enter_finally_section(self, section_id):
    ...
    # TODO(mdanatg): This, not the caller, should track the active sections.
...
class AstToCfg(gast.NodeVisitor):
  ...
  def visit_Try(self, node):
    ...
    if node.handlers:
      # TODO(mdanatg): Should we still support bare try/except? Might be confusing.
  ...
  def visit_With(self, node):
    # TODO(mdanatg): Mark the context manager's exit call as exit guard.
